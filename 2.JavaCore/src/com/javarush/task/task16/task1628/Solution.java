...
public class Solution {
    ...
    synchronized static String read() throws IOException {
    ...
    public static class ReaderThread extends Thread {
        ...
        public void run() {
            ...
            try {
                while (!isInterrupted()) {
                    if (reader.ready()) {
                        result.add(Solution.read());
                        Solution.readStringCount.getAndIncrement();
                        System.out.println("Отработал поток: " + Thread.currentThread().getName() + ". " + Solution.readStringCount.get());
                    }
                }
            } catch (IOException e) {
