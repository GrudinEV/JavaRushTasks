    ...
    public static void sort(List<Stock> list) {
        list.sort(new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                if (!((String) stock1.get("name")).equals((String) stock2.get("name"))) {
                    return ((String) stock1.get("name")).compareTo((String) stock2.get("name"));
                }
                int date1 = Integer.parseInt(df.format((Date) stock1.get("date")));
                int date2 = Integer.parseInt(df.format((Date) stock2.get("date")));
                if (date1 != date2) {
                    return date2 > date1 ? 1 : -1;
                }
                if (((Date) stock1.get("date")).getTime() >= new Date().getTime()) {
